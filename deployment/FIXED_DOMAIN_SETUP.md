...
Important:
- The credentials file is generated by `cloudflared tunnel create`.
- Do not replace that JSON file with output from `cloudflared tunnel token`.
...
```
Then add Startup shortcut (not file copy):
- Press `Win + R` and run: `shell:startup`
- Right-click -> `New` -> `Shortcut`
- Target: your repo `start-tracker.bat` path
